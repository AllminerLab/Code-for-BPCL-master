...
def build_knowledge(training_instances):
    MAX_SEQ_LENGTH = 0
    item_freq_dict = {}
    all_instances=training_instances

    for line in all_instances:
        basket_seq = line.split("|")[:-1]

        if len(basket_seq) > MAX_SEQ_LENGTH:
            MAX_SEQ_LENGTH = len(basket_seq)
    ...
    return MAX_SEQ_LENGTH, item_dict, reversed_item_dict

def build_sparse_adjacency_matrix_v2(training_instances, item_dict):
...
def seq_batch_generator(raw_lines, max_length,item_dict, batch_size=32, is_train=True):
    NB_ITEMS = len(item_dict)
    ...
    while 1:
        ...
        for line in lines:
            ...
            if len(elements)> SEQ_LEN_LMAX:
                elements=elements[-SEQ_LEN_LMAX:]
            ...
            s = np.zeros(shape=(max_length, NB_ITEMS), dtype=np.int32)
            for t, basket in enumerate(bseq):
                item_list = re.split('[\\s]+', basket)
                s[t]=create_binary_vector(item_list,item_dict)
            S.append(np.array(s))
